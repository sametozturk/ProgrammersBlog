                        ...
                        success: function (data) {
                            ...
                            if (categoryListDto.ResultStatus === 0) {
                                ...
                                $.each(categoryListDto.Categories.$values,
                                    ...
                                    <td>${convertFirstLetterUpperCase(category.IsActive.toString())}</td>
                                    <td>${convertFirstLetterUpperCase(category.IsDeleted.toString())}</td>
                                    <td>${category.Note}</td>
                    ...
                    if (isValid) {
                        ...
                        const newTableRow = `
                                <tr>
                                                    ...
                                                    <td>${convertFirstLetterUpperCase(categoryAddAjaxModel.CategoryDto.Category.IsActive.toString())}</td>
                                                    <td>${convertFirstLetterUpperCase(categoryAddAjaxModel.CategoryDto.Category.IsDeleted.toString())}</td>
    ...
    /*Ajax Post / Updating a Category starts from here*/
        ...
        placeHolderDiv.on('click', '#btnUpdate', function (event) {
            ...
            $.post(actionUrl, dataToSend).done(function () {
                ...
                placeHolderDiv.find('.modal-body ').replaceWith(newFormBody);
                /* c# daki == karşılığı burda ===  yani hem değer hem tip eşit olması lazım*/
                const isValid = newFormBody.find('[name="IsValid"]').val() === 'True';
                if (isValid) {
                    placeHolderDiv.find('.modal').modal('hide');
                    const newTableRow = `
                                <tr>
                                                    ...
                                                    <td>${convertFirstLetterUpperCase(categoryUpdateAjaxModel.CategoryDto.Category.IsActive.toString())}</td>
                                                    <td>${convertFirstLetterUpperCase(categoryUpdateAjaxModel.CategoryDto.Category.IsDeleted.toString())}</td>
